...
def read_intrinsics(path):
    with open(path, 'r') as f:
        x = list(map(float, f.read().strip().split()))[:3]
    return x

def read_pose(path):
    with open(path, 'r') as f:
        x = list(map(float, f.read().strip().split()))
...
class NeRFImageDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=10):
        ...
        elif os.path.exists(os.path.join(self.root_path,'pose')):
            self.mode = 'SRN' # provided split
        else:
        ...
        if self.mode == 'colmap':
            ...
            transform_paths = sorted(glob.glob(os.path.join(self.root_path, 'pose', '*.txt')))
            transform_images = sorted(glob.glob(os.path.join(self.root_path, 'rgb', '*.png')))
            frames = []
            for p, im in zip(transform_paths, transform_images):
                im = os.path.relpath(im, self.root_path)
                print('path', p, im)
                pose = read_pose(p)
                print(pose)
                frames.append({'file_path': im,
                               'transform_matrix':read_pose(p)})
        ...
        if 'fl_x' in transform or 'fl_y' in transform:
            ...
            fl, cx, cy = read_intrinsics(os.path.join(self.root_path,'intrinsics.txt'))
            fl_x = fl_y = fl/4
            transform['cx'] = cx/4
            transform['cy'] = cy/4
            print('fl', fl/4, 128 / (2 * np.tan(0.69 / 2)))
        else:
