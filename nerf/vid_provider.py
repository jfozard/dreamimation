...
def rand_poses(size, device, radius_range=[1, 1.5], theta_range=[0, 120], phi_range=[0, 360], return_dirs=False, angle_overhead=30, angle_front=60, jitter=False, uniform_sphere_rate=0.5):
    ...
    if random.random() < uniform_sphere_rate:
        ...
        thetas = torch.acos(unit_centers[:,2])
        phis = torch.atan2(unit_centers[:,0], unit_centers[:,1])
        phis[phis < 0] += 2 * np.pi
...
def circle_poses(device, radius=1.25, theta=90, phi=0, return_dirs=False, angle_overhead=30, angle_front=60):
...
def load_points(filename, scale=4):
    pos, rgb = [], []
    print('load points', filename)
    with open(filename, 'r') as f:
        for l in f:
            if 'v' in l:
                d= list(map(float, l[1:].strip().split(' ')))
            else:
    ...
    rgb = rgb[::2]
    pos = np.array(pos)
    pos = pos - np.mean(pos, axis=0)[None,:]
    return np.array(pos)*scale, np.array(rgb)
